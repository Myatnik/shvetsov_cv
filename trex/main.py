...
def restart_game():
    global region, dino_spawn, spawn_show, detect_len, slam_len, slam_len_left, last_time_detect, last_time_slam, airborn, low
...
#image_over = cv2.imread("over.png") 
...
while True:
    ...
    if detect_len < 400 and current_time1 - last_time_detect >= increment_interval:
        ...
        slam_len += int(detect_len * speed_increment / 2)
        #print("detection rose\n")
        last_time_detect = current_time1
    if current_time2 - last_time_slam >= slam_push_interval:
        slam_len_left += 5
    ...
    roi_crouch = labeled[top_offset+10:top_offset + top_shrink - 1, left_offset + dino_spawn[2] + 10:left_offset + slam_len]
    roi_stand = labeled[0:0, 1:1] #do i need that one? it kinda works even without it

    if np.any(roi_jump>0) and not airborn:
        jump()
    #
    if np.all(roi_slam == 0) and airborn:
        slam()
    #
    ...
    if np.all(roi_crouch == 0) and low:
        stand()
    #finish restart function here (it has very ugly if)
    if labeled.max() > 14:
    ...
    if spawn_show:
        ...
        cv2.rectangle(result, (left_offset + dino_spawn[2] + 10, top_offset+10), (left_offset + slam_len,top_offset + top_shrink - 1), (255, 0, 0), 1)
    #
